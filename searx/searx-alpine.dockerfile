...
ARG APP_CONF_FILENAME="${APP_CONF_FILENAME:-"settings-default.yml"}"                       # main component - config file
...
ENTRYPOINT ["/scripts/entrypoint.sh"]                                                      # shell script switching between modes at runtime
# CMD [""]                                                                                 # available modes: interactive, bash, dev, run
...
ARG APP_DIR_SHARED_PREFIX_PATH="${APP_DIR_SHARED_PREFIX_PATH:-"/${APP_DIR_SHARED_LABEL}"}" # root, /shared
ARG APP_DIR_CONFS="${APP_DIR_CONFS:-"$APP_DIR_SHARED_PREFIX_PATH/conf.d"}"                 # conf.d: config files
ARG APP_DIR_CERTS="${APP_DIR_CERTS:-"$APP_DIR_SHARED_PREFIX_PATH/ssl"}"                    # certificates
ARG APP_DIR_LOGS="${APP_DIR_LOGS:-"$APP_DIR_SHARED_PREFIX_PATH/logs"}"                     # logs generated by app
ARG APP_DIR_DATA="${APP_DIR_DATA:-"$APP_DIR_SHARED_PREFIX_PATH/data"}"                     # data generated by app
ARG APP_DIR_DICTS="${APP_DIR_DICTS:-"$APP_DIR_SHARED_PREFIX_PATH/dicts"}"                  # dataset(s) imported/shared
ARG APP_DIR_LOAD="${APP_DIR_LOAD:-"$APP_DIR_SHARED_PREFIX_PATH/load"}"                     # dataset(s) to bulk load/import
ARG APP_DIR_PLUGINS="${APP_DIR_PLUGINS:-"/shared/plugins"}"

# to export and refined path(s) to share with the container env
ARG APP_DATA_DIR="${APP_DATA_DIR:-"$APP_DIR_DATA/$APP_USER"}"                              # app specific - generated data dir (eg. backup)
ARG APP_LOGS_DIR="${APP_LOGS_DIR:-"$APP_DIR_LOGS/$APP_USER"}"                              # app specific - log dir
ARG APP_LOAD_DIR="${APP_LOAD_DIR:-"$APP_DIR_LOAD"}"                                        # app specific - data to load
...
ARG APP_CONF_FILEPATH="${APP_CONF_FILEPATH:-"$APP_DIR_SHARED/conf.d/$APP_CONF_FILENAME"}"  # main component - config filepath

# install script (will look after install-{SCRIPT_SLUG}.sh)
ARG APP_INSTALL_SCRIPTS="${APP_INSTALL_SCRIPTS:-"pip3 libgit2 searx searx-admin"}"         # list of component(s) to install
