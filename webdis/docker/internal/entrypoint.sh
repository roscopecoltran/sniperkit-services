...
webdis_config="/shared/conf.d/webdis.json"
...
install_redis() {
  if [ -n "$LOCAL_REDIS" ] || [ -n "$INSTALL_REDIS" ]; then
    ...
    redis-server >/shared/logs/redis-server.log 2>&1 &
  fi
}
...
cat - <<EOF
{
  ...
  "logfile": "${LOGFILE:-/shared/logs/webdis.log}"
}
EOF
}
...
if [ $# -eq 0 ]; then
  ...
  tail -F ${LOGFILE:-/shared/logs/webdis.log} &
