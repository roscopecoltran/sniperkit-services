...
gcloud compute instances create provisioner-01 	
gcloud compute disks create ${disk_name}  --size ${SIZE} --type pd-ssd
gcloud compute instances attach-disk provisioner-01 --disk ${disk_name} 

devpath="/dev"
device="$(gcloud compute ssh provisioner-01 --command "sudo lsblk --output name | tail -1")"
