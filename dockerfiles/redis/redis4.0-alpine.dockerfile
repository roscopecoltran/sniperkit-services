...
#		  
#  Build the image:                                               		  
#    $ docker build -t redis:4.0-alpine3.6 --no-cache .    		# longer but more accurate
#    $ docker build -t redis:4.0-alpine3.6 .    					# faster but increase mistakes
#                                                                 		  
#  Run the container:                                             		  
#    $ docker run -it --rm -v $(pwd)/shared:/shared -p 6379:6379 redis:4.0-alpine3.6
#    $ docker run -it -p 6379:6379 -v $(pwd)/shared:/shared --name redis redis:4.0-alpine3.6
#                                                              		  
###########################################################################
...
EXPOSE 6379
# CMD ["redis-server", "${APP_SHARED}/conf.d/${APP_CONF_FILENAME}"]
...
# Set env variables
ARG DOCKER_SERVICES=${DOCKER_SERVICES:-"redis/4.0"}
