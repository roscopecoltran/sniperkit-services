...
#		  
#  Build the image:                                               		  
#    $ docker build -t phoenix -f phoenix-alpine.dockerfile --no-cache . 					# longer but more accurate
#    $ docker build -t phoenix -f phoenix-alpine.dockerfile . 							# faster but increase mistakes
#                                                                 		  
#  Run the container:                                             		  
#    $ docker run -it --rm -v $(pwd)/shared:/shared -p 4242:4242 phoenix
#    $ docker run -d --name phoenix -p 4242:4242 -v $(pwd)/shared:/shared phoenix
#                                                              		  
###########################################################################
...
ARG APACHE_PHOENIX_VERSION=${APACHE_PHOENIX_VERSION:-"4.11.0"}
...
# with gosu
# CMD ["/usr/local/sbin/gosu", "app", "/app/phoenix", "--listen-peer-urls", "http://0.0.0.0:2380", "--listen-client-urls", "http://0.0.0.0:2379"]
