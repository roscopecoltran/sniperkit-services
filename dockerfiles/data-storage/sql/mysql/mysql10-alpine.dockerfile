...
#         
#  Build the image:                                                       
#    $ docker build -t mysql:10-alpine3.6 --no-cache .                                     # longer but more accurate
#    $ docker build -t mysql:10-alpine3.6 .                                                # faster but increase mistakes
#                                                                         
#  Run the container:                                                     
#    $ docker run -it --rm -v $(pwd)/shared:/shared -p 3306:3306 mysql:10-alpine3.6
#    $ docker run -d --name mysql10 -p 3306:3306 -v $(pwd)/shared:/shared mysql:10-alpine3.6
#                                                                     
###########################################################################
...
EXPOSE 3306
# VOLUME ["/shared", "/etc/mysql"]

ARG MYSQL_DATA_DIR=${MYSQL_DATA_DIR:-"/shared/data/mysql"}
ARG MYSQL_LOGS_DIR=${MYSQL_LOGS_DIR:-"/shared/logs/mysql"}
ARG MYSQL_LOAD_DIR=${MYSQL_LOAD_DIR:-"/shared/load"}

ENV MYSQL_DATA_DIR=${MYSQL_DATA_DIR} \
    MYSQL_LOGS_DIR=${MYSQL_LOGS_DIR} \
    MYSQL_LOAD_DIR=${MYSQL_LOAD_DIR}

ENTRYPOINT ["/scripts/entrypoint.sh"]
# CMD [""]

# app configuration
ARG APP_CONF_FILENAME=${APP_CONF_FILENAME:-"my.conf"}
...
ARG APK_RUNTIME_CUSTOM=${APK_RUNTIME_CUSTOM:-"mysql mysql-client pwgen percona-toolkit"}
