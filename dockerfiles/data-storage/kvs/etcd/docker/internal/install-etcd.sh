...
export ETCD_BUILD_DATE=${ETCD_BUILD_DATE:-"$BUILD_DATE"}

### PRE_CHECK #################################################################################################

if [ "${ETCD_VCS_URI}" == '' ]; then
	exit 1
fi
...
pwd
ls -l 
export ETCD_VCS_VERSION=$(git ${BUILD_VCS_VERSION_ARGS:-"describe --always --long --dirty --tags"})
