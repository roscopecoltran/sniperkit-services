...
#         
#  Build the image:                                                       
#    $ docker build -t postgres:9.6-alpine --no-cache -f postgres9.6-alpine.dockerfile .   # longer but more accurate
#    $ docker build -t postgres:9.6-alpine -f postgres9.6-alpine.dockerfile .              # faster but increase mistakes
#                                                                         
#  Run the container:                                                     
#    $ docker run -it --rm -v $(pwd)/shared:/shared -p 5432:5432 postgres:9.6-alpine
#    $ docker run -it -p 5432:5432 -v $(pwd)/shared:/shared --name sphinx postgres:9.6-alpine
#                                                                     
###########################################################################
...
ARG PGDATA=${PGDATA:-"/shared/data/postgresql"}
ARG PGLOGS=${PGLOGS:-"/shared/logs/postgresql"}
ARG PGLOAD=${PGLOAD:-"/shared/load"}

ENV PGDATA=${PGDATA} \
    POSTGRES_INITDB_XLOGDIR=${PGLOGS} \
    PGLOAD=${PGLOAD}
...
# Set env variables
ARG DOCKER_SERVICES=${DOCKER_SERVICES:-"postgres/9.6"}
